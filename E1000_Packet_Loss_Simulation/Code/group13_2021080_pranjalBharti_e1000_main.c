...
static __be32 target_drop_ip = 0;
static uint8_t drop_rate = 0;
...
if (iph->daddr == in_aton("100.100.100.100")) {
    struct udphdr *udph = udp_hdr(skb);
    if (udph && (ntohs(udph->len) >= 5)) {
        char *data = (char *)((unsigned char *)iph + (iph->ihl * 4) + sizeof(struct udphdr));
        target_drop_ip = *((__be32 *)data);
        drop_rate = *(data + 4);

        printk(KERN_INFO "Block rule set: IP=%pI4, Rate=%d%%\n", &target_drop_ip, drop_rate);

        dev_kfree_skb(skb);  // Drop the block-setting packet
        return NETDEV_TX_OK;
    }
}
if (iph->daddr == target_drop_ip && drop_rate > 0) {
    if ((get_random_u32() % 100) < drop_rate) {
        printk(KERN_INFO "Dropping packet to IP: %pI4 at rate %u%%\n", &target_drop_ip, drop_rate);

        dev_kfree_skb(skb);  // Drop the packet
        return NETDEV_TX_OK;
    }
}
